  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!finalId) {
        ...
        const normalized = {
          placeId: raw.id || raw._id || raw.restaurantId || null,
          favId: null,
          title: raw.name || raw.title || raw.restaurantName || 'Restaurante',
          images,
          description: raw.description || raw.content || raw.about || '',
          location: raw.city || raw.location || raw.address || '',
          actionType: 'rate', // use rating flow instead of favorites for restaurants
          initialComments: Array.isArray(raw.reviews) ? raw.reviews.map(c => ({ author: c.author || c.user || 'Usuário', text: c.text || c.body || c.comment || '' })) : (Array.isArray(raw.comments) ? raw.comments.map(c => ({ author: c.author || c.name || 'Usuário', text: c.text || c.body || '' })) : []),
        };
